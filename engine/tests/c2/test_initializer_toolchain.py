# Migrated from cipher-2 tests/test_initializer_toolchain.py (M4 facts absorption acceptance).
...
#   * cipher2.config.{load_config,write_default_config} -> c2.initializer_support.build_config
#     (6-field shim; no config file). `replace(load_config(...), extractor_worker_count=N)` ->
#     build_config(..., extractor_worker_count=N).
#   * cipher2.tools.log.open_log -> the extractor's real jsonl log (arbiter_engine.facts.extractor.code).
#   * cipher2.storage -> arbiter_engine.facts.store; .cipher -> .arbiter/facts (map §1.6).
#
# Band A (~22): hermetic capability/diagnostic/libclang-unit tests on collect()/static helpers.
# Band B (~7): full-pipeline tests driven through the initialize_repository support shim.
# Band C (3): real-toolchain parity tests that self-skip via shutil.which("clang").
import gzip
import json
import shutil
import tempfile
...
def _read_source_inventory_text(target: Path, snapshot_id: str) -> str:
    path = target / ".arbiter" / "facts" / "snapshots" / snapshot_id / "source_inventory.jsonl.gz"
...
class InitializerToolchainTest(unittest.TestCase):
    def test_llvm_clang_17_capability_probe_is_accepted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = write_fake_toolchain(target, clang_version="17.0.6", gcc_version="10.5.0")

            result = CodeFactExtractor(target, config).collect(["main.c"], "default")
    ...
    def test_apple_clang_21_capability_probe_is_accepted_without_gcc(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = build_config(target, clang_executable="bin/clang", gcc_executable=None)
    ...
    def test_gcc_version_is_not_checked_on_ast_only_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp)
            _write(target / "main.c", "int entry(void) { return 0; }\n")
            config = write_fake_toolchain(target, clang_version="16.0.6", gcc_version="11.4.0")
    ...
    def test_clang_capability_probe_rejects_non_json_output(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_gcc(target)
            config = build_config(target, clang_executable="bin/clang", gcc_executable="bin/gcc")

            with self.assertRaises(InitError) as caught:
                CodeFactExtractor(target, config).collect(["main.c"], "default")
    ...
    def test_partial_ast_returncode_error_accepts_valid_ast_with_warning_and_inventory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["main.c"])
    ...
    def test_empty_translation_unit_inner_remains_file_ast_failure(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["empty.c"])
    ...
    def test_file_ast_malformed_warning_records_diagnostic_kind(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["bad.c"])
    ...
    def test_file_ast_timeout_warning_records_diagnostic_kind(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                summary = initialize_repository(target, config=config, source_roots=["slow.c"])
            finally:
    ...
    def test_file_ast_failure_is_warning_and_other_files_continue(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["good.c", "bad.c"])
    ...
    def test_all_file_ast_failures_still_write_snapshot_with_warnings(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["a.c", "b.c"])

            self.assertTrue(summary.ok)
            self.assertEqual(summary.warning_count, 2)
            self.assertEqual(summary.fact_count, 0)
            self.assertTrue((target / ".arbiter" / "facts" / "snapshots" / summary.snapshot_id / "manifest.json").is_file())
    ...
    def test_real_libclang_header_cache_preserves_dual_run_and_worker_parity(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cache_off_wc1 = self._collect_shared_header_shape(target, config, worker_count=1, cache_enabled=False)
            cache_on_wc1 = self._collect_shared_header_shape(target, config, worker_count=1, cache_enabled=True)
            cache_on_wc2 = self._collect_shared_header_shape(target, config, worker_count=2, cache_enabled=True)
    ...
    def _collect_or_skip_real_toolchain(self, target: Path, config, *, backend_factory, skip_codes):
        with mock.patch.object(code_extractor, "_TEST_AST_BACKEND_FACTORY", backend_factory):
            try:
                return CodeFactExtractor(target, config).collect(["src/main.c"], "debug")
    ...
    def _collect_shared_header_shape(self, target: Path, base_config, *, worker_count: int, cache_enabled: bool):
        config = replace(base_config, extractor_worker_count=worker_count)
