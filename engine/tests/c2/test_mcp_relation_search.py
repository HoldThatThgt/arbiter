# Migrated from cipher-2 tests/test_mcp_relation_search.py (M4 acceptance) — the relation-grammar spec.
# Adaptations: open_mcp_server -> open_facts_server (rpc shim); cipher2.storage -> arbiter_engine.facts.store;
# the cipher2.tools.log open_log read_events assertions in the too_broad test are dropped (arbiter runs
# the fact store log-disabled); the overlay fact_view_provider test is skipped (Phase 2 incremental — the
# cwd-bound rpc handler has no overlay-provider hook yet).
import tempfile
import unittest
...
class McpRelationSearchTest(unittest.TestCase):
    def test_readers_file_filter_strips_endpoint_line_suffix(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_facts_server(target).search("readers:NullableDatum.value file:ruleutils.c", limit=20)
    ...
    def test_relation_search_too_broad_is_keyed_by_limit_not_fixed_threshold(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = open_facts_server(target).call_tool("search", {"query": "readers:List.length", "limit": 20})
        ...
        # (dropped: open_log('mcp').read_events returned_ids assertions — arbiter runs the store log-disabled)
        self.assertEqual(
    ...
    def test_file_scoped_too_broad_returns_bounded_answer_guidance(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = open_facts_server(target).call_tool(
                "search",
    ...
    def test_relation_search_transitive_uses_slim_rows_and_no_salience_duplicate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = open_facts_server(target).call_tool("search", {"query": "callees:root depth:2", "limit": 20})
    ...
    def test_reachable_returns_path_complete_and_no_endpoint_rows(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = open_facts_server(target).call_tool("search", {"query": "reachable:root->leaf", "limit": 20})
    ...
    def test_relation_search_invalid_depth_returns_refinement_response(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = open_facts_server(target).call_tool("search", {"query": "callees:root depth:0", "limit": 20})
    ...
    def test_relation_search_hard_error_tool_result_keeps_storage_guidance(self):
        with tempfile.TemporaryDirectory() as tmp:
            result = open_facts_server(Path(tmp)).call_tool(
                "search",
    ...
    def test_unique_exact_function_anchor_ignores_substring_fallback_candidates(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_facts_server(target).search("callers:LWLockAcquire", limit=20)
    ...
    def test_ambiguous_anchor_returns_deterministically_ordered_refinement(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_facts_server(target).search("callers:add_var", limit=20)
    ...
    def test_field_anchor_refinement_lists_candidate_id_owner_and_source(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_facts_server(target).search("writers:value", limit=20)
    ...
    def test_single_fuzzy_anchor_requires_refinement_instead_of_joining_wrong_fact(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_facts_server(target).search("callers:init_var", limit=20)
