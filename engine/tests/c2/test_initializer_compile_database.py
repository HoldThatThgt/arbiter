# Migrated from cipher-2 tests/test_initializer_compile_database.py (M4 facts absorption acceptance).
...
#   * cipher2.storage -> arbiter_engine.facts.store; .cipher -> .arbiter/facts.
#
# Red-line: the `build_overview(...)`/`overview.log.*` dashboard assertions in
# test_compile_database_limits_sources_to_indexed_entries are dropped — cipher2.tools.views is
# excluded (map §3, observability/dashboard). The same facts (compile-command hit/miss/duplicate/
# ignored-outside-repo counts) are still asserted directly against the extractor's real log events,
# which is the data build_overview merely re-summarized.
import json
import tempfile
import unittest
...
class InitializerCompileDatabaseTest(unittest.TestCase):
    def test_per_file_arguments_are_sanitized_and_ordered_after_global_args(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["src/main.c"])
    ...
    def test_compile_database_limits_sources_to_indexed_entries(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["src"])
            ...
            with self.subTest(entries=entries):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    with self.assertRaises(InitError) as caught:
                        initialize_repository(target, config=config, source_roots=["main.c"])
