# Migrated from cipher-2 tests/test_initializer_api.py (M4 facts absorption acceptance).
# Rewrites per docs/proposals/m4-test-migration-map.md:
#   * cipher2.initializer.{InitError,InitSummary,initialize_repository} -> c2.initializer_support
#     (test-support transcription of the orchestrator; no engine analog — map §1.5).
#   * cipher2.config.write_default_config -> c2.initializer_support.build_config (6-field shim; map §1.2/§3).
...
from c2.initializer_support import InitError, InitSummary, build_config, initialize_repository
...
class InitializerApiTest(unittest.TestCase):
    ...
    def test_empty_repository_writes_empty_fact_snapshot_and_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((target / ".arbiter" / "facts" / "snapshots" / "current").exists())
...
""".strip()
            ...
            first = initialize_repository(target, config=config, source_roots=["src/main.c"], profile="debug", log_enabled=False)
            second = initialize_repository(target, config=config, source_roots=["src/main.c"], profile="debug", log_enabled=False)
    ...
    def test_repeated_initialization_with_header_global_keeps_snapshot_id(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                ...
                first = initialize_repository(target, config=config, source_roots=["src"], profile="debug", log_enabled=False)
                second = initialize_repository(target, config=config, source_roots=["src"], profile="debug", log_enabled=False)
    ...
    def test_initialization_streams_without_collect_materialization(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch(
                "arbiter_engine.facts.extractor.code.CodeFactExtractor.collect",
                ...
                summary = initialize_repository(target, config=config, source_roots=["src"], log_enabled=False)
    ...
    def test_explicit_source_roots_and_profile_limit_scanned_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = initialize_repository(target, config=config, source_roots=["src/included.c"], profile="release", log_enabled=False)
