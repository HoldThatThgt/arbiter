"""Append-only JSONL event log for the facts subsystem (extractor + incremental coordinator).

The fact store runs log-disabled (its forensics live in the referee journal — see
``store/_common.open_log``, a deliberate no-op). Two facts subsystems do keep a real audit
trail because their own observability is part of their contract:

- the **extractor** (channel ``initializer``) — its acceptance tests read back
  ``extractor.code.*`` events to assert diagnostic kinds, worker-pool counts, and payload
  sanitization. On-disk root ``.arbiter/facts/log/<channel>.jsonl``.
- the **incremental coordinator** (channel ``incremental``) — its tests read back
  ``incremental.*`` events. On-disk root ``.arbiter/facts/run/log/<channel>.jsonl``.

Each consumer keeps its own base dir (``base_parts``) and channel, so their event streams stay
isolated. This module is faithfully trimmed from cipher-2's ``cipher2.tools.log`` to exactly the
surface those tests read: ``LogEvent`` (+ ``to_json``/``from_json``), ``open_log(target)`` →
``JsonlLog`` with ``write_event`` / ``read_events`` / ``summarize``, and the redaction +
truncation applied on write. (Originally two near-duplicate modules — the extractor's ``_log.py``
and a minimal ``facts/log.py`` — consolidated here.)
"""
...
# CLI invocations. In arbiter each writer is the sole, in-process emitter for its base dir
# (workers return results, never emit events), so a per-path threading.Lock fully serializes
# writes. We deliberately do NOT take a raw OS-level file lock here: arbiter mandates that every
# such lock route through arbiter_engine/shared/locks.py (enforced by tests/test_locks.py), and a
# general-purpose append lock is not part of that ordered-lock vocabulary. Append-atomicity for
# the single writer is preserved by the open("a") + threading.Lock pairing below.


DEFAULT_BASE_PARTS = ("facts", "log")
...
class JsonlLog:
    def __init__(self, target_repo: Path, *, base_parts: Sequence[str] = DEFAULT_BASE_PARTS) -> None:
        self.target_repo = Path(target_repo)
        # The base dir under .arbiter/ for this log's per-channel files. The extractor uses
        # ("facts","log"); the incremental coordinator uses ("facts","run","log") so its run-state
        # audit trail stays beside the rest of run/ and isolated from the extractor's stream.
        self.base_parts = tuple(base_parts)
    ...
    def _log_base(self) -> Path:
        return self.target_repo.joinpath(".arbiter", *self.base_parts)
    ...
    def _channel_path(self, channel: str) -> Path:
        ...
        if not _is_relative_to(path_resolved, base_resolved):
            raise LogError("path_escape", "log path escapes the channel base dir", path=path)
        return path

    def _selected_paths(self, channel: Optional[str]) -> List[Path]:
        if channel is not None:
            return [self._channel_path(channel)]
        base = self._log_base().resolve(strict=False)
        if not base.exists():
            return []
...
def open_log(target_repo: Path, *, base_parts: Sequence[str] = DEFAULT_BASE_PARTS) -> JsonlLog:
    return JsonlLog(Path(target_repo), base_parts=base_parts)
