...
func TestMain(m *testing.M) {
	if os.Getenv("CHESS_TEST_STUB") == "1" {
		runStub()
		return
	}
	os.Exit(m.Run())
...
func TestMCP(t *testing.T) {
	...
	writeMCP(t, root, map[string]any{
		"ok": map[string]any{
			"type":    "stdio",
			"command": stub,
			"env":     map[string]any{"CHESS_TEST_STUB": "1", "CHESS_TEST_MODE": "ok"},
		},
		"bad": map[string]any{
			"type":    "stdio",
			"command": stub,
			"env":     map[string]any{"CHESS_TEST_STUB": "1", "CHESS_TEST_MODE": "bad"},
		},
		"slow": map[string]any{
			"type":    "stdio",
			"command": stub,
			"env":     map[string]any{"CHESS_TEST_STUB": "1", "CHESS_TEST_MODE": "slow"},
		},
		"http": map[string]any{
			"type":    "http",
			"command": stub,
...
func runStub() {
	...
	server.AddTool(&mcp.Tool{Name: "probe", InputSchema: map[string]any{"type": "object"}}, func(ctx context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		switch os.Getenv("CHESS_TEST_MODE") {
		case "bad":
