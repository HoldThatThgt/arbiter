...
func TestSubmitTaskSummaryValidation(t *testing.T) {
	...
	status, err := os.ReadFile(filepath.Join(root, ".chess", "status.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestNotePlaybook(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(root, ".chess", "playbook", "flow.md"))
	if err != nil {
		t.Fatal(err)
	}
...
`
	...
	data, err := os.ReadFile(filepath.Join(root, ".chess", "playbook", "once.md"))
	if err != nil {
		t.Fatal(err)
	}
...
func repoWithBook(t *testing.T, name, body string) string {
	t.Helper()
	root := t.TempDir()
	dir := filepath.Join(root, ".chess", "playbook")
