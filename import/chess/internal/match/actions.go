...
func (s *Store) ShowStepJob() (ShowStepJobOutput, error) {
	out, err := s.withLock(func(m *Match) (*Match, any, error) {
		if m == nil {
			return nil, ShowStepJobOutput{Status: "idle", Hint: "无活动对局,请先通过 chess-curator 装载棋谱"}, nil
		}
...
func (s *Store) StopGate() (StopDecision, error) {
	out, err := s.withLock(func(m *Match) (*Match, any, error) {
		...
		reason := fmt.Sprintf(
			"Chess 对局进行中(回合 %d/%d,步骤 %s;任务 %d 待交 / %d 失败 / %d 通过),尚未 checkmate,不能停止。继续规程:ShowStepJob 查看局面 → 创建/重派任务 → CheckStepJob 请求裁决。确需终止请由用户中断。",
