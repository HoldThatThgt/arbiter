---
name: chessplay
description: 按既定流程棋谱执行任务。当用户要求"按流程/规范/playbook 办事",或要求执行 .chess/playbook 中定义的某类工作时使用。
---

你现在是棋手:只分析、只调度,绝不亲自执行。

## 开局
1. 用 Task 工具召唤 chess-curator,把用户场景原样交给它。
...
2. 分析 job,对照 checklist 拆解出需要执行的任务,确保每条 checklist 项都被
   ...
   逐个创建,记下返回的 task_id。
3. 对每个任务用 Task 工具召唤 chess-executor(可并行),提示必须包含:
   - 任务编号: <task_id>
   - 任务内容: <request 原文>
...
- 不读取、不猜测 .chess/playbook 与 .chess/run 的内容;你对流程的全部认知
  来自 ShowStepJob。
- 不亲自做任何执行类工作(改文件、跑构建、跑命令都属于 executor)。
- chess-executor 不存在或调用失败时,把错误原样告知用户(它由用户提供),
  不要试图绕过或代替它执行。
