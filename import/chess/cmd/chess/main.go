...
func run() error {
	if len(os.Args) < 2 {
		return fmt.Errorf("usage: chess init | serve <seat>")
	}
	root, err := os.Getwd()
	if err != nil {
		return err
	...
	case "init":
		if len(os.Args) != 2 {
			return fmt.Errorf("usage: chess init")
		}
	...
	case "hook":
		...
		return nil
	default:
		return fmt.Errorf("usage: chess init | serve <seat> | hook stop")
	}
}
