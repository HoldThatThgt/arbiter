...
WORKLOADS = [
    {"name": "small", "facts": 1_000, "memory_mb": 5, "timeout_seconds": 20},
    {"name": "medium", "facts": 100_000, "memory_mb": 40, "timeout_seconds": 120},
    {"name": "large", "facts": 1_000_000, "memory_mb": 128, "timeout_seconds": 900},
]
...
def main() -> None:
    ...
    for workload in WORKLOADS:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                manifest = store.replace_facts(_facts(int(workload["facts"])))
                reader = open_fact_store(target, mode="r", log_enabled=False)
                index_started = time.perf_counter()
                first_search = reader.search("alpha", limit=5)
                cold_index_ms = (time.perf_counter() - index_started) * 1000
                search_results = store.search("alpha", limit=5)
                low_limit = store.search("alpha", limit=1)
                stats = store.stats()
            ...
            if manifest.fact_count != workload["facts"]:
                raise AssertionError(f"{workload['name']} manifest count mismatch")
            if stats.total_facts != workload["facts"]:
                raise AssertionError(f"{workload['name']} stats count mismatch")
            if len(first_search) != 5 or len(search_results) != 5 or len(low_limit) != 1:
                raise AssertionError(f"{workload['name']} search limit mismatch")
    ...
    print(json.dumps({"storage_performance_gate": results}, ensure_ascii=False, sort_keys=True))
