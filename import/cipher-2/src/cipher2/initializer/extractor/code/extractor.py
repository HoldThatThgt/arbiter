...
class CodeFactExtractor:
    ...
    def extract_dirty_sources(self, dirty_sources, profile: str):
        from cipher2.incremental import IncrementalBuildResult
    ...
    def _collect_source_files(self, source_roots: Optional[Sequence[Union[str, Path]]]) -> List[Path]:
        ...
        for root in roots:
            ...
            if _is_cipher_path(resolved, target_resolved):
                raise _make_init_error("invalid_source_root", "source root cannot be inside .cipher")
    ...
    def _emit_init_stage(
        ...
        *,
        ...
        if duration_ms is None:
            duration_ms = 0.0 if started is None else _elapsed_ms(started)
        try:
