...
class FileFactStore:
    def __init__(
        ...
        *,
        ...
        stage_sink: Optional[Callable[[str, float, Dict[str, int], Dict[str, JSONValue]], None]] = None,
    ) -> None:
        if mode not in {"r", "w"}:
            raise StorageError("invalid_mode", "mode must be 'r' or 'w'")
        self.target_repo = Path(target_repo)
        self.cipher_dir = self.target_repo / ".cipher"
        self.mode = mode
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                self._emit_stage(
                    "snapshot_write",
                    _elapsed_ms(snapshot_write_started),
                    counts={
            ...
            self._emit_stage(
                "read_index",
                float(read_index_build_ms),
                counts={
                    "fact_count": prepared["fact_count"],
                    "relative_count": prepared["relative_count"],
                    "source_count": prepared["source_count"],
                    "read_index_bytes": int(read_index["bytes_on_disk"]),
                    "read_index_build_ms": int(read_index_build_ms),
                },
                payload={
                    "snapshot_format": SNAPSHOT_FORMAT,
    ...
    def _emit_stage(
        ...
        *,
        ...
        if self.stage_sink is None:
            return
        try:
            self.stage_sink(stage, duration_ms, dict(counts or {}), dict(payload or {}))
        except Exception:
            pass
...
def open_fact_store(
    ...
    *,
    ...
    return FileFactStore(target_repo, mode=mode, log_enabled=log_enabled, stage_sink=stage_sink)
