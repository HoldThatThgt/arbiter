...
def _snapshot_dir(target: Path) -> Path:
    ...
    return target / ".cipher" / "snapshots" / snapshot_id
...
class StorageCorruptionTest(unittest.TestCase):
    ...
    def test_v3_plaintext_snapshot_is_not_compatible(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            snapshot_dir = target / ".cipher" / "snapshots" / snapshot_id
            snapshot_dir.mkdir(parents=True)
            (target / ".cipher" / "snapshots" / "current").write_text(snapshot_id, encoding="utf-8")
    ...
    def test_current_pointer_to_missing_snapshot_is_reported(self):
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp)
            current = target / ".cipher" / "snapshots" / "current"
