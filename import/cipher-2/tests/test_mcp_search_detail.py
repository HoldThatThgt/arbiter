...
from cipher2.mcp import DetailResponse, SearchResponse, open_mcp_server
...
class McpSearchDetailTest(unittest.TestCase):
    def test_search_returns_fact_summaries_with_empty_query_object_id_order(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_mcp_server(target).search("", limit=2)
    ...
    def test_search_tool_call_returns_structured_content_and_text_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = open_mcp_server(target).call_tool("search", {"query": "alpha", "limit": 1})
    ...
    def test_search_exact_identifier_without_exact_object_name_reports_fallback_signal(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = open_mcp_server(target).call_tool("search", {"query": "init_var", "limit": 5})
    ...
    def test_search_returns_owner_qualified_field_fact(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_mcp_server(target).search("Node.type", limit=5)
    ...
    def test_detail_returns_payload_and_source_context_for_object_source_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            response = open_mcp_server(target).detail("fact:alpha", budget="normal")

        self.assertIsInstance(response, DetailResponse)
    ...
    def test_detail_tool_call_returns_not_found_error_result(self):
        with tempfile.TemporaryDirectory() as tmp:
            result = open_mcp_server(Path(tmp)).call_tool("detail", {"fact_id": "missing", "budget": "normal"})
