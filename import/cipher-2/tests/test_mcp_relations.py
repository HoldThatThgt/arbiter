...
class McpRelationsTest(unittest.TestCase):
    def test_relations_tool_call_is_not_public_mcp_interface(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = server.call_tool("relations", {"fact_id": "fact:a", "direction": "outgoing"})
    ...
    def test_detail_preview_exposes_bounded_internal_relation_audit(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            detail = server.call_tool("detail", {"fact_id": "fact:a"})
        ...
        self.assertFalse(any(event.event_name == "mcp.relations" for event in events))
    ...
    def test_detail_preview_high_fan_in_field_readers_keep_counts_and_diversity(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            detail = server.call_tool("detail", {"fact_id": field_id, "budget": "normal"})
            event = next(event for event in open_log(target).read_events(channel="mcp").events if event.event_name == "mcp.detail")
        ...
        self.assertEqual(event.counts["relative_diversity_bucket_count"], 1)
    ...
    def test_detail_preview_rolls_up_call_sites_and_keeps_conditions(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            detail = server.call_tool("detail", {"fact_id": "fact:add_var", "budget": "normal"})
            event = next(event for event in open_log(target).read_events(channel="mcp").events if event.event_name == "mcp.detail")
        ...
        self.assertEqual(event.counts["relative_rollup_group_count"], 3)
        self.assertEqual(event.counts["relative_collapsed_instance_count"], 2)
    ...
    def test_detail_preview_uses_source_diversity_and_missing_endpoint_defaults(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            server = open_mcp_server(target, fact_view_provider=lambda: _FakeFactView(facts, relatives))

            detail = server.call_tool("detail", {"fact_id": "fact:target", "budget": "small"})
            event = next(event for event in open_log(target).read_events(channel="mcp").events if event.event_name == "mcp.detail")

        self.assertFalse(detail.is_error)
        callers = next(bucket for bucket in detail.structured_content["relative_preview"]["buckets"] if bucket["bucket"] == "callers")
        names = [item["endpoint_name"] for item in callers["relatives"]]
        sources = [item["endpoint_source"] for item in callers["relatives"]]
        self.assertEqual(names, ["a0", "a1", "b0", "c0", "d0"])
        self.assertEqual(sum(1 for source in sources if source and source.startswith("src/a.c:")), 2)
        self.assertIn("generated", sources)
        all_names = [item["endpoint_name"] for item in detail.structured_content["relative_preview"]["relatives"]]
        self.assertNotIn("fact:missing", all_names)
        self.assertEqual(event.counts["relative_preview_source_file_count"], 4)
        self.assertEqual(event.counts["relative_diversity_bucket_count"], 1)
    ...
    def test_detail_preview_missing_endpoint_profile_is_null(self):
        ...
        self.assertFalse(detail.is_error)
        caller = detail.structured_content["relative_preview"]["buckets"][0]["relatives"][0]
        self.assertEqual(caller["endpoint_name"], "fact:missing")
        self.assertIsNone(caller["endpoint_source"])
        self.assertIsNone(caller["endpoint_profile"])
    ...
    def test_detail_preview_rollup_and_selection_are_identical_for_overlay_view(self):
        ...
        with tempfile.TemporaryDirectory() as base_tmp, tempfile.TemporaryDirectory() as overlay_tmp:
            ...
            base_detail = open_mcp_server(base_target).call_tool("detail", {"fact_id": "fact:target", "budget": "normal"})
            ...
            overlay_server = open_mcp_server(overlay_target, fact_view_provider=lambda: store.open_view(overlay))
        ...
        self.assertEqual(
            base_detail.structured_content["relative_preview"]["buckets"],
            overlay_detail.structured_content["relative_preview"]["buckets"],
        )
