...
- **`arbiter-engine` (Python ≥3.9, stdlib-only, pip/uv-installed package):** cipher-2 absorbed as `facts/`, crun rebuilt on cipher's MCP loop as `runs/` with harness adapters, plus `shared/` (work-tree census, disk build cache, compile-db management, unified config/logging). **The engine lives outside the target repo** (site-packages), pinned by a machine-local, gitignored `.arbiter/run/engines.json`; an opt-in offline mode embeds it via go:embed with mandatory spawn-time digest verification. (Decision vs the earlier embed-by-default plan: tamper-resistance of the adjudication evaluator and a single version channel outweigh the one-artifact aesthetic, and chess already accepts machine-local paths.)
