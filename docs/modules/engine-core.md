...
- Typed error taxonomy (JSON-RPC error.data.kind): `no_snapshot{hint}`, `briefing_unresolved
  ...
  the synchronous reconcile, ADR-0020), `lock_timeout{lock}`, `internal_error{exception,detail}`
  (the chassis catch-all for unexpected handler exceptions). New kinds require a doc update here +
  transcripts.
